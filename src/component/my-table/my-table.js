...
const MyTable = (props) => {
    ...
    const handleAdd = () => {
        ...
        const colNames = cols.map((col)=>Object.keys(col));
        const newData = {
            key: count,
        };

        for (let i=1; i<cols.length; i++){
            const propertyName = colNames[0];
            newData[propertyName[i]] = "Your data";
            console.log("PN: ", propertyName);
        }
        setDataSource([...dataSource, newData]);
        setCount(count + 1);
        console.log("ND: ",newData);
        console.log("CLN: ",colNames);
    }
